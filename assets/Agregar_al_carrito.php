...
if (mysqli_num_rows($resultado) > 0) {
    ...
    $actualizar_cantidad = "UPDATE carrito_compras SET cantidad = cantidad + '$cantidad' WHERE id_usuario = '$id_usuario' AND id_producto = '$id_producto'";
    mysqli_query($conexion, $actualizar_cantidad);
    $actualizar_subtotal = "UPDATE carrito_compras SET precio_total = precio_total + '$precio_total' WHERE id_usuario = '$id_usuario' AND id_producto = '$id_producto'";
    mysqli_query($conexion, $actualizar_subtotal);
} else {
    // Si el producto no está en el carrito, inserta el nuevo producto y la cantidad
    $insertar = "INSERT INTO carrito_compras (id_usuario, id_producto, cantidad, precio_total) VALUES ('$id_usuario', '$id_producto', '$cantidad', '$precio_total')";
